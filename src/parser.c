...
/* 
输入：网络参数每个 [ 下的所有行组成的list，临时参数params
作用：解析卷积层的参数，并调用 convolutional_layer.c 中的make_convolutional_layer 函数，为layer各个参数分配空间或者初始化
输出：初始化layer结构体中的各个参数，并返回
*/
...
convolutional_layer parse_convolutional(list *options, size_params params)
{
    ...
    int size = option_find_int(options, "size",1);
    int stride = option_find_int(options, "stride",1);
    int pad = option_find_int_quiet(options, "pad",0);  // 为真的时候，由size决定padding的大小
    int padding = option_find_int_quiet(options, "padding",0);
    int groups = option_find_int_quiet(options, "groups", 1);
    if(pad) padding = size/2;   //计算padding的大小， / 的运算结果的数据类型与被除数的数据类型保存一致

    char *activation_s = option_find_str(options, "activation", "logistic");
    ACTIVATION activation = get_activation(activation_s);  // ACTIVATION为枚举类型
...
/*
输入：网络参数每个 [ 下的所有行组成的list，临时参数params
功能：解析全连接层的参数，并调用 connected_layer.c 中的make_connected_layer函数，为layer各个参数分配空间或者初始化
*/
...
/*
输入：字符串 s
功能：将字符串和learning_rate_policy列举的所有类型进行比较，返回比较出的结果
输出：匹配到的枚举类型
*/
...
/*
输入：参数 list，网络 net
功能：对网络的 超参数 进行解析，解析出的结果分别赋值到 network 结构体中
输出：net结构体
返回：无
*/
void parse_net_options(list *options, network *net)
{
    ...
    net->batch /= subdivs;
    net->batch *= net->time_steps;  // 注意这个真实的batch求法
    ...
    if(!net->inputs && !(net->h && net->w && net->c)) error("No input parameters supplied"); // & 和 | 既是逻辑运算符也是位运算符，而&&和||只是逻辑运算符。
    ...
    if(net->policy == STEP){ 
        ...
        int n = 1;
        int i;
        for(i = 0; i < len; ++i){  // 以逗号为分割，查看steps中有多少个数据，用 n 表示
            if (l[i] == ',') ++n;
        }
        int *steps = calloc(n, sizeof(int));
        float *scales = calloc(n, sizeof(float));
        for(i = 0; i < n; ++i){
            int step    = atoi(l); // atoi 遇上数字或正负符号才开始做转换，而在遇到非数字或字符串结束符('\0')才结束转换，并将结果返回
            float scale = atof(p);
            l = strchr(l, ',')+1; //从字符串 l 中寻找字符 ' 第一次出现的位置(指针)
            p = strchr(p, ',')+1;
            steps[i] = step;
            scales[i] = scale;
        }
        net->scales = scales;  // 这里的 net->scales 是一个指针，之前并未分配大小，这里的 scales 已经分配内存了，将 net->scales 指向 scales 的地址
        net->steps = steps;
        net->num_steps = n;
...
/*
输入：网络配置文件名
功能：解析网络配置文件中的参数，其中超参数赋值给net，而
*/
network *parse_network_cfg(char *filename)
{
    list *sections = read_cfg(filename);  // 读取网络配置文件，并存到list中，list的 noede 中 val 中存储的为 section 类型
    node *n = sections->front;
    if(!n) error("Config file has no sections");
    network *net = make_network(sections->size - 1);  // 这里的参数指的是网络层的个数，而第一个 [] 是网络的超参数，所以需要减去1
    net->gpu_index = gpu_index;  //gpu_index 在 cuda.c 中定义
    size_params params;

    section *s = (section *)n->val;
    list *options = s->options;     // section结构体中的 options list存储的是 每个[] 下的所有语句
    if(!is_network(s)) error("First section must be [net] or [network]");
    parse_net_options(options, net);  // 对网络的超参数进行解析

    // 对 params 中的所有变量值进行初始化
    params.h = net->h;
    params.w = net->w;
    params.c = net->c;
    ...
    n = n->next;
    int count = 0;
    free_section(s); //释放此时的 s 结构体所占用的内存，因为 node *n 目前还在使用，所以不能释放
    ...
    while(n){
        ...
        s = (section *)n->val;
        options = s->options;
        layer l = {0};  // 这里的 l 是一个临时变量，对其进行初始化，注意 layer 中有很多种类型
        LAYER_TYPE lt = string_to_layer_type(s->type); // 匹配出 layer 的类型
        if(lt == CONVOLUTIONAL){
            ...
            l.output = net->layers[count-1].output; // dropout的上一层
        ...
        option_unused(options); // 每次执行上面 option_find_ 函数都会将节点中的每一项的used置为1，这行打印没有解析过的语句
        net->layers[count] = l; // 之前 make_network 函数分配了一个指针大小，这里将该指针指向了 l，l在上述子函数中已经分配了大小
        if (l.workspace_size > workspace_size) workspace_size = l.workspace_size; // 在 cuda 中使用
        free_section(s);
        n = n->next;
        ++count;
        if(n){   // 将当前层的输出大小和输出数据作为下一层的输入，赋值给params中的临时变量
            params.h = l.out_h;
            params.w = l.out_w;
            params.c = l.out_c;
...
输入：文件名
功能：读取网络配置文件
返回：一个链表options，options存储所有的section，每一个section的 type存储 []，section的 list 存储 [] 对应的每一行参数
*/
list *read_cfg(char *filename)
{
    ...
    char *line;
    int nu = 0;
    list *options = make_list(); // options 是存储网络配置的链表
    section *current = 0;
    while((line=fgetl(file)) != 0){
        ...
        switch(line[0]){
            case '[':
                current = malloc(sizeof(section)); // current的type存储 []，而current的options链表存储的是 每一个[] 下对应的参数
                list_insert(options, current); // 在options中存储所有的 current
            ...
            case ';':
                ...
                break;
            default:
                if(!read_option(line, current->options)){ // 将每一个 [] 下的所有行以 = 分割，并存储到 current->options 中

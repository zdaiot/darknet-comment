...
/*
输入：网络的层数 n
功能：为网络进行初始化，为其分配内存空间
输出：初始化内存空间后的 network
*/
network *make_network(int n)
{
    network *net = calloc(1, sizeof(network)); // network结构体中含有 layer，这是一个指针，calloc分配 layer 的时候，按照指针所占用的大小分配
    net->n = n;
    net->layers = calloc(net->n, sizeof(layer)); // 为network中的 layer 指针分配大小
...
#endif
    network net = *netp;  // netp 是network类型的指针，*表示取其值赋值给net变量
    int i;
    for(i = 0; i < net.n; ++i){
        net.index = i;
        layer l = net.layers[i];
...
void backward_network(network *netp)
{
    ...
    network net = *netp;
    int i;
    network orig = net;   // 保留最原始的net，反向传播到输入层的时候要用到
    // 网络一共有net.n层，从最后一层反向传播，因为C下标从0开始，因此第一个值为net.n-1
    for(i = net.n-1; i >= 0; --i){
        layer l = net.layers[i];  // 声明一个临时layer指针，指向的是当前层
        if(l.stopbackward) break;
        // 如果是第0层的话，也就是输入层，net.input 直接就是网络的 net.input，然后也就不需要计算 net.delta，值为NULL。进入l.backward函数后net.delta的值进行判断，不再计算 net.delta
        if(i == 0){
            net = orig;  // 最原始的net
        }else{
            layer prev = net.layers[i-1];  // 声明一个layer指针，指向的是前一层
            net.input = prev.output; // 将前一层的输出作为该层的输入
            net.delta = prev.delta;   // 传入用于计算前一层 delta 值的前一项，然后在前一层 l.backward中计算完整的 delta
        }
        net.index = i;
        l.backward(l, net);
    }

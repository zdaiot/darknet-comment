...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    int count = 0;
    //while(i*imgs < N*120){
    while(get_current_batch(net) < net->max_batches){  // 每次输入一个batch
...
{
    list *options = read_data_cfg(datacfg); //这里读取的是 cfg/coco.data 文件
    char *name_list = option_find_str(options, "names", "data/names.list"); // 这里char* 指向的仍然是 options 中匹配的内存
    char **names = get_labels(name_list); // 得到类标的类型，并存储到names中，names[]中存储的是每一个类标字符串的地址

    image **alphabet = load_alphabet(); // 加载模板图片
    network *net = load_network(cfgfile, weightfile, 0);
    set_batch_network(net, 1);
    srand(2222222);
    double time;
